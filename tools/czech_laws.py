...
class CzechLaws(BaseTool):
    ...
    class InputModel(BaseModel):
        law_name: str = Field(description = """The name of the law in Czech Republic. 
    ...
    def _scrape_law(self, url: str) -> Tuple[Dict, List[Dict]]:
        headers = {
    ...
    def _convert_paragraphs_to_string(self, paragraphs: List[Dict]) -> str:
    ...
    def __call__(self, law_name: str) -> Dict:
        """Get the law of the Czech Republic"""
    
        if law_name in self.laws_dict:
            url = self.laws_dict[law_name]
            _, paragraphs = self._scrape_law(url)
            law_text = self._convert_paragraphs_to_string(paragraphs)
