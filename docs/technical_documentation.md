...
`adapters.serializers.provider_dump()` uses ordinary Pydantic JSON serialization with aliases and exclusion of unset fields. Only OpenAI parsed-response wrappers are handled specially: `ParsedResponse.output`, `ParsedResponseOutputMessage.content`, and `ParsedResponseOutputText.parsed` are excluded from the wrapper dump and serialized separately when explicitly set. This preserves concrete parsed payload serializers and explicit nulls without enabling global runtime-type serialization, which can fail on deferred SDK serializers such as reasoning summaries. SDK imports remain lazy; other provider objects retain their existing serialization behavior, and warnings and errors propagate. Offline coverage in `tests/test_openai_response_serialization.py` and `tests/test_provider_serialization.py` includes reasoning summaries in a fresh process, citations/refusals/tool calls, persistence/replay, Pydantic semantics, and mocked sync/async facade requests.
