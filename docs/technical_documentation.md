...
- `GoogleAdapter`
  ...
  - Generation parameters (`temperature`, `max_output_tokens`, `thinking_level` for Gemini 3 / `thinking_budget` otherwise, `thinking_summaries: "auto"`, plus `image_config={"aspect_ratio": ..., "image_size": ...}` for image generation) go inside `generation_config`. Structured output is sent as **two separate** top-level params: `response_mime_type="application/json"` plus `response_format=<JSON schema dict>` (the polymorphic `{type, mime_type, schema}` shape from the public docs is not what this SDK version sends on the wire). Image-generation modality is selected via top-level `response_modalities=["image"]`.
